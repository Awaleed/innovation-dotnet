...
- `resources/js/pages/Admin/Challenges/` -- 4 pages + 7 form steps + 13 tabs + dialogs
- `resources/js/layouts/admin-layout.tsx` -- admin sidebar layout
- `resources/js/components/` -- 82 reusable UI components
...
- `src/Innovation.Web/Controllers/Admin/` -- thin Inertia controllers for page rendering
- `src/Innovation.Web/Endpoints/` -- minimal API endpoints for CRUD operations
- `src/Innovation.Web/ClientApp/src/Pages/Admin/` -- React pages (copy from Laravel)
- `src/Innovation.AppHost/Program.cs` -- add database resource

---

## Part 10.5: Inertia.js Integration Patterns

### Controller -> Page Communication
```
.NET Controller                           React Page
──────────────                           ──────────
Inertia.Render("Admin/Challenges/Index",  →  Pages/Admin/Challenges/Index.tsx
  new { challenges, meta })                   receives as typed props
```

### HandleInertiaRequests middleware (shared props on every request)
- `auth` { user, roles, permissions } -- from Keycloak claims
- `localization` { currentLocale, direction } -- AR/EN
- `flash` { success, error } -- session flash messages
- `sidebarOpen` -- sidebar state

### Form submission patterns (from git history of deleted auth pages)
```tsx
// Pattern 1: useForm (Inertia built-in)
const { data, setData, post, processing, errors, reset } = useForm({ email: '', password: '' });
post('/login', { onFinish: () => reset('password') });

// Pattern 2: useInertiaForm (custom hook at hooks/use-inertia-form.ts)
// Extends useForm to auto-sync server validation errors from props.errors

// Pattern 3: router.post for API calls (used in challenge forms)
router.post('/api/v1/challenges', formData, {
  forceFormData: true,  // multipart for file uploads
  onSuccess: () => toast.success('Created'),
  onError: (errors) => toast.error('Failed'),
});
```

### Route helpers (auto-generated by Innovation.TsGen)
```ts
export const dashboard = (): RouteDefinition<'get'> => ({ url: '/dashboard', method: 'get' });
dashboard.url = () => '/dashboard';
// Usage: router.get(dashboard.url()) or <Link href={dashboard.url()}>
```

### Page resolution (App.tsx)
```ts
const pages = import.meta.glob('./Pages/**/*.tsx', { eager: true });
// "Admin/Challenges/Index" resolves to ./Pages/Admin/Challenges/Index.tsx
```

### Cleanup needed
- Auth pages (Login.tsx, Register.tsx) already deleted in commit cc32e59
- `layouts/auth/` directory has unused remnants -- safe to delete
- `components/auth/` directory unused -- safe to delete
- `routes/index.ts` has stale login/register helpers -- needs regeneration
