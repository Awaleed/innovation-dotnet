...
{
    public static ChallengeDetailResponse ToDetailResponse(this Challenge c, string locale = "en") => new(
        Id: c.Id,
        ...
        Awards: c.Awards?.OrderBy(a => a.OrderIndex).Select(a => new AwardResponse(
            a.Id, a.Name.GetOrNull(locale), a.Description?.GetOrNull(locale),
            a.AwardType, a.Value, a.OrderIndex)).ToList() ?? [],
        Objectives: c.Objectives?.OrderBy(o => o.OrderIndex).Select(o => new ObjectiveResponse(
            o.Id, o.Objective.GetOrNull(locale), o.Description?.GetOrNull(locale),
            o.OrderIndex)).ToList() ?? [],
        Requirements: c.Requirements?.OrderBy(r => r.OrderIndex).Select(r => new RequirementResponse(
            r.Id, r.Requirement.GetOrNull(locale), r.Description?.GetOrNull(locale),
            r.RequirementType, r.Mandatory, r.OrderIndex)).ToList() ?? [],
        Timeline: c.Timeline?.OrderBy(t => t.OrderIndex).Select(t => new TimelineResponse(
            t.Id, t.MilestoneName.GetOrNull(locale), t.Description?.GetOrNull(locale),
            t.MilestoneStartDate?.ToString("yyyy-MM-dd"), t.MilestoneEndDate?.ToString("yyyy-MM-dd"),
            t.Status, t.OrderIndex)).ToList() ?? [],
        Sponsors: c.Sponsors?.OrderBy(s => s.OrderIndex).Select(s => new SponsorResponse(
            s.Id, s.SponsorName, s.LogoUrl, s.WebsiteUrl,
            s.ContributionAmount, s.SponsorshipType, s.OrderIndex)).ToList() ?? [],
        CreatedAt: c.CreatedAt.ToString("o"),
        UpdatedAt: c.UpdatedAt.ToString("o"));

    public static ChallengeEditResponse ToEditResponse(this Challenge c) => new(
        Id: c.Id,
