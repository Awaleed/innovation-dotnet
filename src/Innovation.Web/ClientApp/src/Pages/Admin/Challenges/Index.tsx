...
import {
  ...
  getChallengeStatusVariant,
  useChallengeDifficultyLabel,
  useChallengeStatusLabel,
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Badge variant={getChallengeStatusVariant(status)}>{getStatusLabel(status)}</Badge>
          );
        },
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Badge
              variant="outline"
              className={`border-${getChallengeDifficultyColor(difficulty)}-200 text-${getChallengeDifficultyColor(difficulty)}-800`}
            >
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.featured ? <Trophy className="h-4 w-4 text-accent" /> : '-',
      },
      {
        id: 'actions',
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            <Button asChild variant="ghost" size="sm">
              <Link href={admin.challenges.show.url({ id: row.original.id })}>
                <Eye className="h-4 w-4" />
              </Link>
            </Button>
            <Button asChild variant="ghost" size="sm">
              <Link href={admin.challenges.edit.url({ id: row.original.id })}>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          <div className="space-y-2">
            <h3 className="font-heading leading-none font-semibold tracking-tight text-primary">
              {challenge.title}
            </h3>
            <div className="flex gap-2">
              <Badge
                variant={getChallengeStatusVariant(challenge.status as unknown as ChallengeStatus)}
              >
              ...
              {challenge.difficulty && (
                <Badge
                  variant="outline"
                  className={`border-${getChallengeDifficultyColor(challenge.difficulty as unknown as ChallengeDifficulty)}-200 text-${getChallengeDifficultyColor(challenge.difficulty as unknown as ChallengeDifficulty)}-800`}
                >
        ...
        <div className="space-y-1 font-sans text-sm text-primary/70">
          ...
          {challenge.maxParticipants && (
            <div className="flex items-center gap-1">
              <Users className="h-3 w-3 text-secondary" />
              {t('models:challenge.max_participants')}: {challenge.maxParticipants}
            </div>
          )}
        </div>
      </CardHeader>
      <CardContent className="pt-0">
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="flex-1 font-sans" asChild>
            <Link href={admin.challenges.show.url({ id: challenge.id })}>
              <Eye className="me-2 h-4 w-4" />
              {t('common:view')}
            </Link>
          </Button>
          <Button variant="outline" size="sm" className="flex-1 font-sans" asChild>
            <Link href={admin.challenges.edit.url({ id: challenge.id })}>
      ...
      <div className="container mx-auto space-y-6 p-8">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <Card className="border">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <div className="font-sans text-sm font-medium text-primary">
                {t('models:challenge.total')}
              </div>
              <Calendar className="h-4 w-4 text-secondary" />
            </CardHeader>
            <CardContent>
              <div className="font-heading text-2xl font-bold text-primary">
                {challenges?.totalCount ?? 0}
              </div>
            </CardContent>
          </Card>
          <Card className="border">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <div className="font-sans text-sm font-medium text-primary">
                {getStatusLabel(ChallengeStatus.Open)}
              </div>
          ...
          <Card className="border">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <div className="font-sans text-sm font-medium text-primary">
                {getStatusLabel(ChallengeStatus.Draft)}
              </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <Button asChild className="font-sans">
            <Link href={admin.challenges.create.url()}>
