          ...
          <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={
                  advancing || challenge.status === 'Completed' || challenge.status === 'Cancelled'
                }
            ...
            {(challenge.description?.en || challenge.description?.ar) && (
              <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                ...
                {challenge.description?.ar && (
                  <p className="mt-2 text-sm leading-relaxed text-gray-700" dir="rtl">
                    {challenge.description.ar}
                  </p>
                )}
              </div>
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <dl className="grid gap-x-6 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <Field
                  ...
                  value={formatDate(challenge.submissionDeadline)}
                />
              ...
              <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <ul className="space-y-2">
                  <li className="flex items-center gap-2 text-sm text-gray-700">
                    <CheckCircle
                      className={`h-4 w-4 ${challenge.featured ? 'text-green-500' : 'text-gray-300'}`}
                    />
                    Featured
                  </li>
                  <li className="flex items-center gap-2 text-sm text-gray-700">
                    <Globe
                      className={`h-4 w-4 ${challenge.isPublic ? 'text-green-500' : 'text-gray-300'}`}
                    />
                    Public
                  </li>
                  <li className="flex items-center gap-2 text-sm text-gray-700">
                    <MessageCircle
                      className={`h-4 w-4 ${challenge.enableComments ? 'text-green-500' : 'text-gray-300'}`}
                    />
                    Comments Enabled
                  </li>
                </ul>
