...
const getMemberSchema = (t: (key: string) => string) =>
  z.object({
...
export function TeamMemberManager({
  ...
  const handleAddMember = () => {
    if (value.length >= maxMembers) {
      console.error(
        `${t('ui/team-member-manager:cannot_add_more')} ${maxMembers} ${t('ui/team-member-manager:members')}`,
      );
      return;
    }
          ...
          {member.member_type && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground">
                {t('ui/team-member-manager:type')}:
              </span>
              <Badge variant="outline" className="text-xs">
                {MEMBER_TYPES.find((type) => type.value === member.member_type)?.label ||
                  member.member_type}
              </Badge>
            </div>
          )}
    ...
    <div className={cn('space-y-4', className)}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {description || t('ui/team-member-manager:description')}
          </p>
        </div>
        ...
        <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
          ...
          <SheetContent side="left" className="w-full sm:max-w-lg overflow-y-auto">
            <SheetHeader>
              <SheetTitle>
                {editingIndex !== null
                  ? t('ui/team-member-manager:edit_member')
                  : t('ui/team-member-manager:add_new_member')}
              </SheetTitle>
              <SheetDescription>
              ...
              <form onSubmit={form.handleSubmit(handleSaveMember)} className="space-y-4 mt-6">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          {...field}
                          placeholder={t('ui/team-member-manager:member_name_placeholder')}
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                    ...
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} value={field.value}>
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue
                              placeholder={t('ui/team-member-manager:member_type_placeholder')}
                            />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          {...field}
                          placeholder={t('ui/team-member-manager:role_placeholder')}
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                ...
                <div className="flex gap-3 pt-4">
                  <Button type="submit" className="flex-1">
                    {editingIndex !== null
                      ? t('ui/team-member-manager:save_changes')
                      : t('ui/team-member-manager:add_member_button')}
                  </Button>
      ...
      {value.length > 0 ? (
        <div className="grid gap-4 md:grid-cols-2">
          {value.map((member, index) => renderMemberCard(member, index))}
        </div>
      ) : (
        <Card className="border-dashed">
          <CardContent className="pt-6">
            <div className="text-center py-8">
              ...
              <Button onClick={handleAddMember} disabled={disabled} size="sm">
                <Plus className="h-4 w-4 me-2" />
      ...
      {value.length > 0 && (
        <div className="flex justify-between items-center text-sm text-muted-foreground">
          <span>
            {value.length} {t('ui/team-member-manager:members_count')}
          </span>
          <span>
            {t('ui/team-member-manager:max_limit')}: {maxMembers}
          </span>
        </div>
      )}
    </div>
